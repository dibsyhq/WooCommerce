...
class WC_Dibsy_Webhook_Handler extends WC_Dibsy_Gateway_Abstract
{
    ...
    private function generateSignature($secret, $method, $uri, $timestamp, $body)
    {
        $hmac = hash_init('sha256', HASH_HMAC, $secret);
        hash_update($hmac, $method . $uri . $timestamp);
        hash_update($hmac, $body);
        return hash_final($hmac);
    }


    private function verfiySignature($signature, $secret, $method, $uri, $timestamp, $body)
    {
       
        try {
            $generatedSignautre = $this->generateSignature(
                $secret,
                $method,
                $uri,
                $timestamp,
            ...
            return hash_equals($signature,$generatedSignautre);
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    public function process_webhook_payment($body)
    {
        ...
        $order_id = $body_decoded->metadata->order_id;
        $order = new WC_Order($order_id);

        
        if (!$order) {
            WC_Dibsy_Logger::log('Webhook - Could not find order with ID: ' . $order_id);
            return;
        }
        ...
        WC_Dibsy_Logger::log( "Info: (Webhook) Begin processing payment for order $order_id for the amount of {$order->get_total()}" );

        if (strtolower($body_decoded->status) === "succeeded") {
            $order->update_status('processing');
            $order->payment_complete($transaction_id);
            $order->set_transaction_id($transaction_id);
            $order->reduce_order_stock();
            $order->add_order_note(sprintf("Dibsy transaction complete \nTransaction ID: %s ", $transaction_id));
            WC_Dibsy_Logger::log('Webhook - Transaction complete with status succeeded, Transaction ID : ' . $transaction_id);
        } else {
            $order->update_status('failed');
            $order->add_order_note('Dibsy was unable to process the transaction');
            WC_Dibsy_Logger::log('Webhook - Dibsy transaction failed : ' . $transaction_id);
        }
